...
export async function decryptField(privateKey: CryptoKey, field: EncryptedField): Promise<string> {
  // 1. Unwrap AES-GCM key using RSA-OAEP-SHA256
  const wrappedKeyBuf = base64ToUint8Array(field.key);
  const aesKeyRaw = await crypto.subtle.decrypt({ name: "RSA-OAEP" }, privateKey, wrappedKeyBuf);
...
export async function tryDecryptField(privateKey: CryptoKey, value: unknown): Promise<unknown> {
  ...
  try {
    ...
    console.error(`Failed to decrypt field: ${err instanceof Error ? err.message : String(err)}`);
    return value;
  }
}
