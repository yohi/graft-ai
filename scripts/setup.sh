...
AI_GW_LIST=$(env -u CLOUDFLARE_API_TOKEN npx wrangler ai gateway list 2>/dev/null \
...
env -u CLOUDFLARE_API_TOKEN npx wrangler deploy --config "$TAIL_WRANGLER"
success "Tail Worker デプロイ完了"

info "Proxy Worker をデプロイ中..."
env -u CLOUDFLARE_API_TOKEN npx wrangler deploy --config "$PROXY_WRANGLER"
