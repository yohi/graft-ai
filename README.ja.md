...
[workers/src/proxy.ts]
  ...
  └─ "_graft_ai_telemetry": true 付きの JSON telemetry line を1 request につき1行出力
       ↓ Tail Worker logs
...
client は `https://gateway.ai.cloudflare.com/v1/{account_id}/{gateway_id}/...` を直接呼ばず、proxy Worker URL を呼ぶ必要があります。proxy Worker は以下の non-secret な**ルーティング値**から AI Gateway URL を再構築します。

- `CF_ACCOUNT_ID` - Cloudflare account ID（上流 URL パスの構築に使用）
- `AI_GATEWAY_ID` - URL パス内の AI Gateway ID

以下の値は **Loki の低カーディナリティ label のみ**に使用され、ルーティングには必要ありません。
...
Tail Worker を先にデプロイし、その後に tail consumer を設定した proxy Worker をデプロイします。
...
デプロイ後、proxy Worker 経由で client request を1回送信し、Grafana Cloud Loki に `model`、`status_code`、`env`、`gateway` のみを label に持つ log stream が届くことを確認してください。

> **Note:** `make deploy` と `make validate` は Terraform を実行するため、Logpush mode 専用です。Free Tier mode では上記の `npx wrangler deploy` コマンドで Workers を直接デプロイしてください。
