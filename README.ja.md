...
client は `https://gateway.ai.cloudflare.com/v1/{account_id}/{gateway_id}/...` を直接呼ばず、proxy Worker URL を呼ぶ必要があります。proxy Worker は以下の non-secret 値から AI Gateway URL を再構築します。

- `CF_ACCOUNT_ID` - Cloudflare account ID
- `AI_GATEWAY_ID` - URL 内の AI Gateway ID
- `GATEWAY_NAME` - Loki label 用の低カーディナリティ値。通常は `main` などの gateway 名
- `ENV_LABEL` - Loki label 用の低カーディナリティ値。例: `prod`、`staging`
