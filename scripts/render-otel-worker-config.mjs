...
export const OTEL_KV_NAMESPACE_SENTINEL =
  "__OTEL_PAYLOAD_KV_NAMESPACE_ID__";
...
export function renderOtelWorkerConfig(
  ...
  if (!payloadStores.has(payloadStore)) {
    throw new Error(`payloadStore must be one of: kv, r2 (received ${payloadStore})`);
  }
  if (!namespaceIdPattern.test(kvNamespaceId ?? "")) {
    throw new Error("KV namespace ID must be exactly 32 hexadecimal characters");
  }
  ...
  rendered.vars = {
    ...(rendered.vars ?? {}),
    OTEL_PAYLOAD_STORE: payloadStore,
  };
  rendered.kv_namespaces = [
    { binding: "OTEL_PAYLOAD_KV", id: kvNamespaceId },
  ];
...
function parseCliArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (
      argument === "--payload-store" ||
      argument === "--kv-namespace-id" ||
      argument === "--output"
    ) {
      ...
      index += 1;
      if (argument === "--payload-store") options.payloadStore = value;
      if (argument === "--kv-namespace-id") options.kvNamespaceId = value;
